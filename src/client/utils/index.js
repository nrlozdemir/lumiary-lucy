...
const percentageBeautifier = (value) => {
  if (parseInt(value) === 0) {
    return value
  }

  value = Math.round(parseFloat(value).toFixed(2) * 100) / 100
...
const converter = (tempBucket) => {
  if (isObject(tempBucket)) {
    Object.keys(tempBucket).map((el, i) => {
      if (isNumber(tempBucket[el])) {
        tempBucket[el] = percentageIt(tempBucket[el]) 
      }
      else if (isArray(tempBucket[el])) {
        tempBucket[el].forEach((item, index) => {
          if (isNumber(tempBucket[el][index])) {
            tempBucket[el][index] = percentageIt(item) 
          } else {
            tempBucket[el][index] = converter(tempBucket[el][index])
          }
        })
      }
      else if (isObject(tempBucket[el])) {
        Object.keys(tempBucket[el]).map((key, k) => {
          tempBucket[el][key] = converter(tempBucket[el][key])
        })
      }
    })
  } else if (isArray(tempBucket)){
    tempBucket.forEach((item, index) => {
      if (isNumber(tempBucket[index])) {
        tempBucket[index] = percentageIt(item) 
      }
      else if (isArray(tempBucket[index])) {
        tempBucket[index].forEach((arrayItem, arrayIndex) => {
          if (isNumber(tempBucket[index][arrayIndex])) {
            tempBucket[index][arrayIndex] = percentageIt(arrayItem) 
          } else {
            tempBucket[index][arrayIndex] = converter(tempBucket[index][arrayIndex])
          }
        })
      }
      else if (isObject(tempBucket[index])) {
        Object.keys(tempBucket[index]).map((key, k) => {
          tempBucket[index][key] = converter(tempBucket[index][key])
        })
      }
    })
  }
