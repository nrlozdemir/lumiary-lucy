...
export default class oAuthHelper {
      ...
      ajax({
        ...
        .then(function(response) {
          if (
            !response.data ||
            !response.data.brand ||
            response.data.brand.uuid !== brandUuid
          ) {
  ...
  requestAccessToken({ code }) {
    ...
    return new Promise((resolve, reject) => {
      ...
      bodyFormData.set(
        'redirect_uri',
        `${WINDOW_LOCATION_ORIGIN}${INSTAGRAM_REDIRECT_URI}`
      )
    ...
    switch (this.platform) {
        ...
        return new Promise((resolve, reject) => {
          axios({
            ...
            .then(function(response) {
              ...
              if (!oauth_token) {
                throw new Error(
                  'unable to derive oauth_token from azazzle response'
                )
              }
      ...
      case 'instagram':
        ...
        return new Promise((resolve, reject) => {
          ...
          try {
            window.location = `https://api.instagram.com/oauth/authorize/?client_id=${INSTAGRAM_CLIENT_ID}&redirect_uri=${encodeURIComponent(
              `${WINDOW_LOCATION_ORIGIN}${INSTAGRAM_REDIRECT_URI}`
            )}&response_type=${INSTAGRAM_RESPONSE_TYPE}`
          } catch (error) {
            return reject(error)
          }
        })
