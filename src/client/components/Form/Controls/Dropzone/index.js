...
class DropzoneDisplay extends React.Component {
  ...
  render() {
    ...
    return (
      <div className={ cx(style.dropzone, { [style.dropzoneHasImage]: !!backgroundImage }) } style={{backgroundImage: `url(${backgroundImage })`}}>
        {
...
class Dropzone extends React.Component {
  ...
  handleUpload = (data) => {
    const {
      dialog, updateEditItem, tmp,
      uploading, handleUpload, editItem
    } = this.props
    const { signedUrl, file: { type, name, size } } = data
    const asset = {
    ...
    const updatedTmp = editItem ?
      [{ ...tmp[0], ...asset }] : tmp.concat(asset)
    const updatedEditItem = editItem ?
      update(editItem, { data: { $merge: asset }}) : null
  ...
  handleDrop = (accepted) => {
    ...
    handleDrop({
      ...
      uploading: Array.from({ length: accepted.length },
        () => true)
    })

    if (!editItem) {
      uniqueModalOpen !== null ?
        toggleModal() : toggleDropzoneModal()
    }
  }
  ...
  renderUploadFilesDialog = (dialog) => {
    const {
      toggleDropzoneModal, progress,
      uploading, modalOpen,
      uniqueModalOpen, toggleModal
    } = this.props

    return (
      <Dialog
        isOpen={uniqueModalOpen !== null
          ? uniqueModalOpen : modalOpen}
        ...
        confirmProps={{
          ...
          belowButtons: (
            <a className="cancel-dialog" onClick={() => this.handleFinished(false)}>
              I changed my mind.
            </a>
          )
        }}>
  ...
  render() {
    const {
      ...
      dropzoneClassName, handleUploadCustom,
      handleDropCustom, handleProgressCustom,
      s3Url, prompt, children, multiple,
    ...
    return(
      <div className={ style.dropzoneContainer }>
        ...
        <DropzoneS3Uploader
          onFinish={ handleUploadCustom ?
            handleUploadCustom : this.handleUpload }
          onDrop={ handleDropCustom ?
            handleDropCustom : this.handleDrop }
          multiple={multiple}
          onProgress={ handleProgressCustom ?
            handleProgressCustom : this.handleProgress }
          notDropzoneProps={[ 'onDrop', 'onFinish', 'isImage', 'notDropzoneProps' ]}
          s3Url={ s3Url || 'https://s3.amazonaws.com/quickframe-media-dev/' }
          className={ cx(style.dropzoneInner, className) }>
            { children ? (
              children
            ) : (
              <DropzoneDisplay prompt={ prompt } backgroundImage={ backgroundImage }/>
            )}
        </DropzoneS3Uploader>

        { dialog &&
          this.renderUploadFilesDialog(dialog)
        }
      </div>
    )
  }
