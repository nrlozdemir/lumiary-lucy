...
import { Fieldset, Legend, Dropzone, TextField, Textarea, Input, UploadFiles } from 'Components/Form/Controls'
...
class UploadDescription extends React.Component {
  ...
  nextSlide = () => {
    ...
    if(lastSlide) {
      this.setState({
        ...
        handleDescSubmit(merge(
          items,
          currentFormValues ? (currentFormValues.items || []) : []
        ));
      })
    } else {
  ...
  afterSlideChange = (slideIdx) => {
    const { state: { slideCount }} = this.slider.innerSlider
  ...
  render() {
    ...
    return (
      ...
      >
          { items.map((item, idx) =>
          {
            const imgUrl = 'application' != getMimeType(item.url) ? item.url : `${staticUrl}img/no_preview_icon.svg`
              ...
              <div className={style.pane}>
                <div className={ style.imagePane } >
                  <div className={ style.previewImg }>
                    { false
                      ? (
                          <div className={cx(style.dropzoneEditArea, {[style.uploading]: uploading})}>
                            { uploading && (
...
const mapStateToProps = state => ({
  currentFormValues: state.form.uploadDescriptionForm ? state.form.uploadDescriptionForm.values : {},
});
