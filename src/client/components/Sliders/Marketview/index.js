...
class MarketViewSlider extends React.Component {
	...
	settings = {
			...
			return (
				<div className={classnames(style.pagination, 'pagination')}>
					{items.map((item, i) => {
						...
						return (
							<div key={i} onClick={() => this.refSlider.slideTo(i)} className={i === 0 ? 'active' : ''}>
								<img src={item.image} />
								<span><i className={socialIcon}></i>{item.socialMedia}</span>
							</div>
						)
					})}
				</div>
		...
		on: {
			slideChange: (index) => {
				const bullets = this.refSlider.$el[0].querySelector('.pagination').children;
		...
		return (
			<div className={style.section}>
				<div className="marketViewSlider">
					<Swiper ref={node => node && (this.refSlider = node.swiper)} {...this.settings}>
