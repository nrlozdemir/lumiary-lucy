...
const PanopticFlipCards = (props) => {
  ...
  const statSelected = Object.values(stat).filter(word => word.label === selected)
  const statSelectedPrev = Object.values(stat).filter(word => word.label === selectedPrev)
  const statDifference = statSelected[0].score - statSelectedPrev[0].score
  const statDifferenceValue = ((statDifference < 0) ? (statDifference * -1) : statDifference) + '%'
  ...
  if(statDifference === 0){
    ...
    backText = `${props.title} increased today from yesterday, from ${statSelectedPrev[0].score}k to ${statSelected[0].score}k`
  }
  else if(statDifference < 0){
    statArrowClassName = classnames(styles.arrow, styles.arrowDown)
    statClassName = classnames(styles.stats, styles.decrease)
    backText = `${props.title} decreased today from yesterday, from ${statSelectedPrev[0].score}k to ${statSelected[0].score}k`
  }
            ...
            <div className={styles.content}>
              ...
              <div className={styles.changes}>
                <div className={styles.circle}>
                  <i className={statArrowClassName}></i>
                </div>
